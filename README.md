...
| ファイル | 中身 |
|---|---|
| `test/appview_wiring_test.cljk` | ビルド設定・配信される文書・そこへマウントするコードを**繋いでいる文字列**の突き合わせ。依存ゼロの nbb で走り、`npm install` もビルドも要らない。パッケージの中に置けないのは、不変条件がどれも 2 ファイルにまたがっていて、片方だけを持つファイルが無いため |
